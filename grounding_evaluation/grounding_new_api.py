...
from languagebind.image.modeling_image import LanguageBindImage
from languagebind.image.tokenization_image import LanguageBindImageTokenizer
from languagebind.image.processing_image import LanguageBindImageProcessor
from tqdm import tqdm
import json
...
args_original=argparse.Namespace(
    ...
    GROUNDING_DINO_CONFIG_PATH = "/home/hetianyao/GroundingDINO/groundingdino/config/GroundingDINO_SwinB_cfg.py",
    GROUNDING_DINO_CHECKPOINT_PATH = "/home/hetianyao/GroundingDINO/weights/groundingdino_swinb_cogcoor.pth",
    DINO_THRESHOLD = 0.3,
...
class Tracker_with_GroundingDINO:
    def __init__(self, config,
        ...
        self.image_model = LanguageBindImage.from_pretrained('LanguageBind/LanguageBind_Image', cache_dir='./cache_dir').to(device)
        self.tokenizer = LanguageBindImageTokenizer.from_pretrained('LanguageBind/LanguageBind_Image', cache_dir='./cache_dir')
        self.image_process = LanguageBindImageProcessor(self.image_model.config, self.tokenizer)
    ...
    def __classify(self, pre_processed_imgs, zeroshot_weights):
        with torch.no_grad():
            ...
            # predict
            image_features_clipout = self.image_model.vision_model(pixel_values=images)['pooler_output']
            zeroshot_weights = torch.cat([zeroshot_weights, zeroshot_weights], dim=0)
            # image_features_clipout = self.clip_model.encode_image(images)
            image_features = image_features_clipout / image_features_clipout.norm(dim=-1, keepdim=True)
            ...
            return clip_scores, probs, image_features_clipout
    ...
    def __apply_clip_thresholding(self, image_rgb, detections, class_list, clip_zeroshot_weights):
        ...
        if crops:
            ...
            clip_scores, probs, image_clip_features = self.__classify(pre_processed_imgs, clip_zeroshot_weights)
            ...
            return detections, image_clip_features[thresholded_idx]
        else:
            return detections
    ...
    def detect_and_segment_on_frame(self,
        ...
        if detections.xyxy.shape[0] > 0:
            detections, img_clip_features = self.__apply_clip_thresholding(image, detections, self.CLASSES, self.clip_zeroshot_weights)
        else:
            img_clip_features = None
        ...
        for i in np.flip(np.argsort(detections.area)):
            ...
            if mask.sum() > 0:
                ...
                segments_info.append(ObjectInfo(id=curr_id, category_id=class_id, score=confidence, clip_feature=img_clip_features[i][None, :]))
                curr_id += 1
        
        if img_clip_features == None:
            if len(segments_info) != 0:
                print('Mismatched!')
        
        elif len(segments_info) != img_clip_features.shape[0]:
            print('Mismatched!')
    ...
    def process_frame_fn(self,
        ...
        #         mask, segments_info, img_clip_features = self.detect_and_segment_on_frame(image_np, prompts, new_min_side)
        ...
        if self.tracker_cfg['temporal_setting'] == 'online':
