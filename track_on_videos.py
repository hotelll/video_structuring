...
if __name__ == '__main__':
    video_dir = "/mnt/sdb/dataset/msrvtt_qa/video_frames"
    # with open("/home/hetianyao/Video-LLaVA-PG/datasets/MSRVTT_QA/scene_graph.jsonl", "r") as f:
    #     video_sgs = json.load(f)
    with open("/home/hetianyao/Video-LLaVA-PG/datasets/MSRVTT_QA/scene_graph.jsonl", "r", encoding="utf-8") as file:
        video_sgs = [json.loads(line) for line in file]

    class_id_dict = {}
    for video in video_sgs:
        video_key = video["video_name"][:-4]
    ...
    output_dict = {}
    for subdir in tqdm(video_subdirs):
        try:
            ...
            for i in range(frame_num):  # iterate over scenes
                ...
                for class_id, class_label in enumerate(class_list):  # iterate over classes
                    ...
                    if len(tmp_id_to_obj) > 0:
                        image_clip_features = torch.cat([v.clip_feature for k, v in tmp_id_to_obj.items()], dim=0)
                ...
                # if len(comb_tmp_id_to_obj) != len(comb_embeds):
                #     print(1)

                frame_info = {
                    'embed_dir': embed_dir,
                    'tmp_id_to_obj': comb_tmp_id_to_obj,
                    # 'obj_to_tmp_id': comb_obj_to_tmp_id,
                    # 'segments_info': comb_segments_info,
    ...
    with open('/home/hetianyao/Video-LLaVA-PG/datasets/MSRVTT_QA/video_structure_v1.json', 'w') as f:
